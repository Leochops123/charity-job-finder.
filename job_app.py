...
st.success("✅ Last 24 Hours • Multi-Source • Debugging Mode")
...
def is_within_24h(text: str) -> bool:
    if not text:
        return True
    text = text.lower()
    recent = ["today", "just posted", "1 day ago", "hours ago", "24 hour", "minutes ago"]
...
def scrape_charityjob(keyword: str, location: str) -> List[Dict]:
    try:
        ...
        st.caption(f"CharityJob: Found {len(cards)} cards")
...
def scrape_indeed(keyword: str, location: str) -> List[Dict]:
    try:
        ...
        st.caption(f"Indeed: Found {len(cards)} cards")
